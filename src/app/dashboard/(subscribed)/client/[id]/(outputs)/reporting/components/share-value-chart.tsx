...
import { generateShareValueSeries } from "@/lib/businesses/utils";
...
// Takes: An array of BusinessKeyPeopleShareholders objects.
export function ShareValueChart({
  ...
  const chartData = useMemo(
    () => generateShareValueSeries(businesses),
    [businesses]
  );
  ...
  const chartConfig = useMemo(() => {
    ...
    businesses.forEach((b) => {
      ...
      i++;
      b.shareholders.forEach((s) => {
        config[`${toCamelCase(b.name)}-${toCamelCase(s.name)}`] = {
          label: `${b.name} - ${s.name}`,
          color: `var(--chart-${i})`,
        };
        i++;
